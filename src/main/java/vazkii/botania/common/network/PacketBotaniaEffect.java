...
public class PacketBotaniaEffect implements IMessage {
			...
			Minecraft.getMinecraft().addScheduledTask(new Runnable() {
				...
				public void run() {
					...
					case PAINT_LENS: {
						...
						for (int i = 0; i < 10; i++) {
							BlockPos pos = new BlockPos(message.x, message.y, message.z)
									.offset(EnumFacing.VALUES[world.rand.nextInt(6)]);
							Botania.proxy.sparkleFX(
									pos.getX() + (float) Math.random(), pos.getY() + (float) Math.random(),
									pos.getZ() + (float) Math.random(),
					...
					case ITEM_SMOKE: {
						...
						for (int i = 0; i < p; i++) {
							...
							item.world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL,
									message.x + item.world.rand.nextFloat() * item.width * 2.0F - item.width
											- d0 * d3,
									message.y + item.world.rand.nextFloat() * item.height - d1 * d3,
									message.z + item.world.rand.nextFloat() * item.width * 2.0F - item.width
											- d2 * d3,
									d0, d1, d2);
						}
						break;
					...
					case SPARK_MANA_FLOW: {
						...
						double rc = 0.45;
						Vector3 thisVec = Vector3.fromEntityCenter(e1).add((Math.random() - 0.5) * rc,
								(Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
						Vector3 receiverVec = Vector3.fromEntityCenter(e2).add((Math.random() - 0.5) * rc,
								(Math.random() - 0.5) * rc, (Math.random() - 0.5) * rc);
						...
						if (te instanceof TileTerraPlate) {
							...
							for (int i = 0; i < totalSpiritCount; i++) {
								...
								Botania.proxy.wispFX(x, y, z, colorsfx[0], colorsfx[1], colorsfx[2],
										(float) Math.random() * 0.1F + 0.1F, (float) (Math.random() - 0.5) * 0.05F,
										(float) (Math.random() - 0.5) * 0.05F,
										(float) (Math.random() - 0.5) * 0.05F, 0.9F);

								if (ticks == 100)
									for (int j = 0; j < 15; j++)
										Botania.proxy.wispFX(message.x + 0.5, message.y + 0.5, message.z + 0.5,
					...
					case FLUGEL_EFFECT: {
						...
						if (entity != null) {
							for (int i = 0; i < 15; i++) {
								...
								Botania.proxy.wispFX(x, y, z, (float) Math.random(), (float) Math.random(),
					...
					case DIVA_EFFECT: {
						...
						for (int i = 0; i < 50; i++)
							Botania.proxy.sparkleFX(x + Math.random() * target.width,
									y + Math.random() * target.height, z + Math.random() * target.width, 1F, 1F,
									0.25F, 1F, 3);
						break;
					}
					}
	...
	private static float[] makeColor(int color) {
	...
	public enum EffectType {
		...
		TERRA_PLATE(3), // Args: color1, color2, progress
						// (yes this can be made "cheaper" but probably doesn't matter much)
